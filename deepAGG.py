...
def setup():
    ...
    parser.add_argument("--output_dir", default="./output_1/", type=str)
...
# 模型
class DeepAGG(nn.Module):
    def __init__(self, p, q, a, k_p=3, k_q=3):
        super(DeepAGG, self).__init__()
        self.p = p
        self.q = q
        self.k_p = k_p
        self.k_q = k_q
    ...
    # 根据众包工作者的投票M和每个人的回答正确概率Y，得到最终每个问题的预测答案
    @torch.no_grad()
    def ref(self, M, Y):
        G_hat = torch.ones((M.shape[0], self.q, self.a))
        for i, (Y_batch, M_batch) in enumerate(zip(Y, M)):
            ...
            for j in range(self.q):
                for r in range(self.a):
                    ...
                    G_hat[i][j][r] = torch.prod(Y_batch[pos_inds[0], j]) * torch.prod(
                        (1 - Y_batch[neg_inds[0], j]) / (self.a - 1)
                    )
        G_hat = torch.argmax(G_hat, 2)
        return G_hat
...
if __name__ == "__main__":
    ...
    train_dataset = DeepAGGDataset(train_data, num_responses, args.p, args.q, args.runs)
    test_dataset = DeepAGGDataset(test_data, num_responses, args.p, args.q, args.runs)
    ...
    # 模型训练
    model = DeepAGG(args.p, args.q, num_responses, args.k_p, args.k_q)
    ...
    for epoch in range(args.epochs):
        ...
        for M, G in tqdm_bar:
            loss = model(M, G_true=G, nIter=args.nIter)
            ...
            tqdm_bar.set_postfix({"train_loss": loss.item(), "train_acc": acc.item()})
        ...
        with torch.no_grad():
            for M, G in tqdm_bar:
                ...
                tqdm_bar.set_postfix({"test_loss": loss.item(), "test_acc": acc.item()})
